...
**After completing all questions in a category**, you MUST follow this sequence:

1. **Summarize** what you captured for this category
2. **Write to session record**: Append this category's information to `design-session.md`
3. **Checkpoint Confirmation**: Invoke the `checkpoint-sync` skill (it will auto-detect and sync with design-session.md)
4. **Move to next category**: Announce the next category and ask the first question
